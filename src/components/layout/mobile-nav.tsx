"use client";

import Link from "next/link";
import { useState } from "react";

const nav = [
  { href: "/araclar", label: "Araçlar" },
  { href: "/sat", label: "Aracını Sat" },
  { href: "/hakkimizda", label: "Hakkımızda" },
];

export function MobileNav() {
  const [open, setOpen] = useState(false);

  return (
    <div className="md:hidden">
      <button
        type="button"
        onClick={() => setOpen(!open)}
        className="rounded-lg border border-zinc-700 p-2 text-zinc-400 transition hover:border-zinc-500 hover:text-white"
        aria-label={open ? "Menüyü kapat" : "Menüyü aç"}
        aria-expanded={open}
      >
        {open ? (
          <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
          </svg>
        ) : (
          <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" />
          </svg>
        )}
      </button>

      {open && (
        <nav className="absolute left-0 right-0 top-full border-b border-zinc-800 bg-zinc-950/95 px-4 py-4 backdrop-blur-md">
          <div className="flex flex-col gap-1">
            {nav.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                onClick={() => setOpen(false)}
                className="rounded-xl px-4 py-3 text-sm text-zinc-300 transition hover:bg-zinc-900 hover:text-white"
              >
                {item.label}
              </Link>
            ))}
            <Link
              href="/sat"
              onClick={() => setOpen(false)}
              className="mt-2 rounded-full bg-brand-500 px-4 py-3 text-center text-sm font-semibold text-white hover:bg-brand-400"
            >
              Aracını Sat
            </Link>
          </div>
        </nav>
      )}
    </div>
  );
}
