...
export default async function AdminDashboardPage() {
  ...
  const [vehicleCount, availableCount, pendingSubmissions, unreadSubmissions, unreadTestDrives, unreadContactMessages, recentSubmissions, recentTestDrives, recentContactMessages] =
    await Promise.all([
  ...
  return (
    <div>
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-6">
        ...
        <StatCard label="Ungelesene Kontaktanfragen" value={unreadContactMessages} href="/admin/kontaktanfragen" />
      </div>
      ...
      <section className="mt-10">
        <h2 className="font-semibold text-white">Neueste ungelesene Kontaktanfragen</h2>
        {recentContactMessages.length === 0 ? (
          <p className="mt-4 text-sm text-zinc-500">Keine ungelesenen Kontaktanfragen.</p>
        ) : (
          <div className="mt-4 space-y-2">
            {recentContactMessages.map((m) => (
              <Link
                key={m.id}
                href={`/admin/kontaktanfragen/${m.id}`}
                className="flex items-center justify-between rounded-xl border border-zinc-800 px-4 py-3 transition hover:border-zinc-700"
              >
                <div>
                  <p className="line-clamp-1 font-medium text-white">{m.message}</p>
                  <p className="text-xs text-zinc-500">{m.customerName}</p>
                </div>
                <span className="text-xs text-zinc-500">
